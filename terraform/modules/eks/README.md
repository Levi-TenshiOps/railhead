...
**Node group.** A node-level IAM role carries `AmazonEKSWorkerNodePolicy`, `AmazonEC2ContainerRegistryReadOnly`, and `AmazonEKS_CNI_Policy`. The managed node group itself is 2x `t3.medium` on-demand instances (AL2023), scaling 2-4.
...
**Cost note:** the control plane (a flat hourly rate) and the 2x `t3.medium` node group are the billable pieces of this module — both are destroyed at the end of each working session (see the root README's Cost approach section).
