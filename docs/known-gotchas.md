...
During a single-pod partition *every* `/items` request returned 5xx, which makes the arithmetic exact rather than approximate: the numerator becomes the entire `/items` rate, so the per-pod error ratio equals **one minus the `/metrics` share** at every scrape. The ratio therefore stopped tracking the fault and started tracking the traffic mix. Re-queried from Prometheus over the nine minutes the fault was fully expressed, at the 30s interval the rule actually evaluates on: the `/metrics` share moved between **45.0% and 56.3%**, so the ratio moved between **0.438 and 0.550**, with a **median of exactly 0.500** and a mean of 0.502 — sitting on a `> 0.5` threshold rather than above it. The first `PENDING` period was abandoned mid-count when it dipped, resetting the `for: 2m` timer. Detection took **13m52s** against a predicted 4-6 minutes, and needed two attempts. (All of these were re-queried rather than read off the alerts page, which is why the low end is 0.438 and not the 0.471 the page happened to be showing — see #36.)

The delay is not the problem. **A ratio sitting on the threshold means the alert can fail to fire at all** — on a shorter fault, if the scrape interval were tightened, or if real traffic dropped, the median lands below 0.5 and every `PENDING` gets abandoned. The pod would be serving nothing but errors and nothing would say so.
