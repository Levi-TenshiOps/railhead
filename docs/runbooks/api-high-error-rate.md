...
- **Postgres itself unhealthy**, not just unreachable — e.g. its own PVC full, or a bad password after a config change (ArgoCD re-rendering the Bitnami chart can rotate its auto-generated password out from under an already-initialized pod; this repo pins that password to a Terraform-managed Secret specifically to prevent it — see the comment at `terraform/modules/argocd/main.tf:33`. Worth ruling out if the underlying infra was touched recently).
...
**When this still escalates to a human.** Two guards refuse and post a `:no_entry:` message to Slack — 3+ quarantines within 15 minutes (a bad deployment, not one bad pod), and quarantining that would leave zero pods serving traffic. Treat either exactly like any other page.

**The third case does not refuse, and this is the one to know at 3am.** More than one pod alerting at once is a shared failure — quarantining cannot help, because every replacement inherits the same fault. The remediator is meant to refuse here and **does not**: the pods usually arrive in separate Alertmanager payloads, so each one looks like a lone failure ([`known-gotchas.md`](../known-gotchas.md) #32). Expect it to quarantine them anyway, one per `group_interval`. `MIN_REMAINING_READY` still prevents it emptying the Service, so this degrades rather than outages — but **you are the one who has to stop the churn.** Fix the shared dependency; deleting the quarantined pods without fixing it just gives the remediator fresh ones to quarantine.
