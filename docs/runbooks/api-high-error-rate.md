...
- **Postgres unreachable.** The single most likely cause given this app's shape — every `/items` call needs a DB connection, `/health` doesn't. If only `/items` is failing while `/health` stays clean, this is almost certainly it. Check `kubectl get pods -n railhead -l app.kubernetes.io/name=postgresql` and `kubectl logs` on the api pods for `psycopg2.OperationalError`. (This is exactly the failure mode used to test this alert end-to-end: Postgres was deliberately taken offline and hit with a burst of traffic to trigger it for real. The closely-related per-pod fault injection is recorded step-by-step in [`docs/remediator-t2-trigger-validation.md`](../remediator-t2-trigger-validation.md).)
